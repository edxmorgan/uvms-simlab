...
class OmplPlanner:
    def __init__(self, 
        ...
        self.env_bounds = tuple(float(v) for v in env_bounds)
        ...
        x_min, x_max, y_min, y_max, z_min, z_max = self.env_bounds
    ...
    def _valid_with_fcl(
        ...
        max_abs_roll: float,
        max_abs_pitch: float,
        prediction_start_xyz: np.ndarray,
        dynamic_obstacle_prediction_speed: float,
        state,
    ):
        ...
        t_offset = self._dynamic_obstacle_time_offset(
            pw,
        ...
        return planner_world.is_state_valid_xyz(pw, t_offset=t_offset)
    ...
    @staticmethod
    def _dynamic_obstacle_time_offset(
        xyz: np.ndarray,
        prediction_start_xyz: np.ndarray,
        dynamic_obstacle_prediction_speed: float,
    ) -> float:
        speed = max(0.0, float(dynamic_obstacle_prediction_speed))
        if speed <= 1e-9:
            return 0.0
        start = np.asarray(prediction_start_xyz, dtype=float).reshape(3)
        point = np.asarray(xyz, dtype=float).reshape(3)
        return float(np.linalg.norm(point - start) / speed)
    ...
    def plan_se3_path(
        ...
        prediction_start_xyz = np.asarray(start_xyz, dtype=float).reshape(3)
        dynamic_obstacle_prediction_speed = max(0.0, float(dynamic_obstacle_prediction_speed))
        x_min, x_max, y_min, y_max, z_min, z_max = self.env_bounds
        checker = ob.StateValidityCheckerFn(
            partial(
                self._valid_with_fcl,
                self._resolve_planner_world(self.rclpy_node),
                self.safety_margin,
                x_min,
                x_max,
                y_min,
    ...
    def plan_vehicle(
        ...
        *,
        ...
        return self._planner.plan_se3_path(
            ...
            dynamic_obstacle_prediction_speed=float(dynamic_obstacle_prediction_speed),
        )
