...
class UVMSBackend:
    def __init__(self, node: Node, urdf_string: str, vehicle_target_frame: str, task_target_frame: str):
        ...
        self.fcl_world = FCLWorld(urdf_string=urdf_string, world_frame=self.world_origin, vehicle_radius=0.4)
        ...
        self.init_robot_timer = self.node.create_timer(0.1, self._init_backend_states)
    ...
    def _init_backend_states(self):
        ok = self.setup_initial_robot_configuration()
        if ok:
            if self.init_robot_timer is not None:
                self._init_timers()
                self.init_robot_timer.cancel()
                self.init_robot_timer = None
    ...
    def vehicle_target_tf_timer_callback(self):
        ...
        vehicle_target_t = backend_utils.get_broadcast_tf(stamp_now, self.target_vehicle_pose, self.world_origin, self.vehicle_target_frame)
    ...
    def plan_vehicle_trajectory(self):
        ...
        pose_now = self.robot_selected._pose_from_state_in_frame(self.tf_buffer, self.world_origin)

        # Build a pose from the current NED state, then query NWU for planning
        start_xyz = [pose_now.position.x, pose_now.position.y, pose_now.position.z]
        
        start_quat_wxyz = [pose_now.orientation.w, pose_now.orientation.x, pose_now.orientation.y, pose_now.orientation.z]
    ...
    def viz_timer_callback(self):
        ...
        # min_marker, max_marker = backend_utils.visualize_min_max_coords(min_coords, max_coords, self.bottom_z, self.world_origin)
    ...
    #             target_frame=self.world_origin,
    ...
    #             f"Waiting for TF {self.world_origin} <- {self.world_bottom_frame}: {ex}"
    #         )
    #         return
    ...
    def setup_initial_robot_configuration(self):
        ...
        self.current_vehicle_pose = self.robot_selected._pose_from_state_in_frame(self.tf_buffer, self.world_origin)
        self.node.get_logger().info(
            f"ps_out *********************************************: {self.current_vehicle_pose}"
        )
        if self.current_vehicle_pose is None:
            return False
        
        p_initial_world, rot_initial_world = PoseX.from_pose(
            xyz=[self.current_vehicle_pose.position.x, self.current_vehicle_pose.position.y, self.current_vehicle_pose.position.z],
            rot=[self.current_vehicle_pose.orientation.w, self.current_vehicle_pose.orientation.x, self.current_vehicle_pose.orientation.y, self.current_vehicle_pose.orientation.z],
            rot_rep="quat_wxyz",
            frame="NWU",
        ).get_pose(frame="NWU", rot_rep="euler_xyz")
